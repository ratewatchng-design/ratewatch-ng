...
def get_user(telegram_id: int) -> dict | None:
    try:
        ...
        # IMPORTANT: do not swallow this silently — a failed lookup must
        # never be treated the same as "user does not exist", or callers
        # will create duplicate documents for the same telegram_id.
        logger.error(f"get_user lookup failed for {telegram_id}: {e}")
        raise
